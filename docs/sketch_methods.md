...
```text
S_F H = sum_r alpha_r S_r H + sum_m beta_m S_m H
L_F H = H - S_F H
```
...
`metapath_sketch.enabled: true` adds beta-weighted operators to the fused Laplacian:

```text
S_m H = S_step_l ... S_step_1 H
S_F H = sum_r alpha_r S_r H + sum_m beta_m S_m H
```
...
Use `configs/sketch_chebheat_metapath.yaml` for the default Chebyshev heat setup and `configs/sketch_chebheat_metapath_tiny.yaml` for a tiny synthetic smoke configuration with one `user-item-user` meta-path operator.
