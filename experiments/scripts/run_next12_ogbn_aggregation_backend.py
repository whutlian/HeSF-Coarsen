...
from experiments.scripts.summarize_next12_ogbn_aggregation_backend import summarize_next12_ogbn_aggregation_backend


BACKENDS: dict[str, dict[str, Any]] = {
    "A0_current_sort_reducer": {"aggregation_reducer": "sort", "aggregation_chunk_size": 1_000_000},
    "A3_packed_key_sort": {"aggregation_reducer": "packed_key_sort", "aggregation_chunk_size": 1_000_000},
}
...
def _run_name(size: str, method: str, backend: str) -> str:
    return f"next12_ogbn_backend_{backend}_{size}_{method.replace('-', '_')}"
...
def _rows_from_run(run_dir: Path, size: str, method: str, backend: str, status: str, reason: str = ""):
    ...
    run = {
        ...
        "correctness_passed": "true" if status == "available" and all(float(rel.get("edge_weight_abs_error", 0) or 0) <= 1.0e-5 for rel in rels if isinstance(rel, Mapping)) else "false",
        "edge_weight_preservation_checks": "passed" if status == "available" and all(float(rel.get("edge_weight_abs_error", 0) or 0) <= 1.0e-5 for rel in rels if isinstance(rel, Mapping)) else "failed",
        "run_dir": str(run_dir),
    }
    stage = {key: agg.get(key, "") for key in ("aggregation_relation_loop_sec", "aggregation_assignment_map_sec", "aggregation_key_build_sec", "aggregation_sort_sec", "aggregation_reduce_sec", "aggregation_dedup_sec", "aggregation_kway_merge_sec", "aggregation_output_write_sec", "aggregation_total_sec")}
    stage.update({"size": size, "method": method, "backend": backend})
    by_relation = []
    for rel in rels:
        if isinstance(rel, Mapping):
            by_relation.append({**rel, "size": size, "method": method, "backend": backend})
    ...
    return run, stage, by_relation, check


def _copy_a0(next11_summary: Path, sizes: Sequence[str], methods: Sequence[str]):
    runs = []
    stages = []
    rels = []
    checks = []
    for row in read_csv(next11_summary / "aggregation_optimizer_runs.csv"):
        if row.get("aggregation_variant") == "A0_current_sort_reducer" and row.get("size") in sizes and row.get("method") in methods:
            runs.append({**row, "backend": "A0_current_sort_reducer"})
            checks.append({"size": row.get("size", ""), "method": row.get("method", ""), "backend": "A0_current_sort_reducer", "correctness_passed": "true", "edge_weight_preservation_checks": "passed", "reason": "reused_next11_a0"})
    for row in read_csv(next11_summary / "aggregation_optimizer_stage_breakdown.csv"):
        if row.get("aggregation_variant") == "A0_current_sort_reducer" and row.get("size") in sizes and row.get("method") in methods:
            stages.append({**row, "backend": "A0_current_sort_reducer"})
    for row in read_csv(next11_summary / "aggregation_optimizer_by_relation.csv"):
        if row.get("aggregation_variant") == "A0_current_sort_reducer" and row.get("size") in sizes and row.get("method") in methods:
            rels.append({**row, "backend": "A0_current_sort_reducer"})
    return runs, stages, rels, checks


def run_next12_ogbn_aggregation_backend(
    *,
    sizes: Sequence[str],
    ...
    if reuse_a0_from and reuse_a0_from.exists() and "A0_current_sort_reducer" in backends:
        a0 = _copy_a0(reuse_a0_from, sizes, methods)
        runs.extend(a0[0])
        stages.extend(a0[1])
        rels.extend(a0[2])
        checks.extend(a0[3])
    ...
    for backend in backends:
        if backend == "A0_current_sort_reducer" and reuse_a0_from and reuse_a0_from.exists():
            continue
        ...
        for method in methods:
            for size in sizes:
                ...
                write_command_metadata(run_dir, run_name=run_name, command=command, status="running", size=size, method=method, backend=backend, experiment_block="next12_ogbn_aggregation_backend")
                ...
                write_command_metadata(run_dir, run_name=run_name, command=command, status="success" if completed.returncode == 0 else "failed", returncode=completed.returncode, size=size, method=method, backend=backend, experiment_block="next12_ogbn_aggregation_backend")
                run, stage, rel_rows, check = _rows_from_run(run_dir, size, method, backend, status, reason)
                runs.append(run)
                stages.append(stage)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--backends", nargs="+", default=["A0_current_sort_reducer", "A3_packed_key_sort"])
    parser.add_argument("--candidate-mode", default="optimized")
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--reuse-a0-from", type=Path, default=Path("outputs/exp_next11_ogbn_aggregation_optimizer_20260517_summary"))
    parser.add_argument("--progress", action="store_true")
    args = parser.parse_args(argv)
    run_next12_ogbn_aggregation_backend(sizes=args.sizes, methods=args.methods, backends=args.backends, output=args.output, python=args.python, reuse_a0_from=args.reuse_a0_from, progress=bool(args.progress))
    summarize_next12_ogbn_aggregation_backend(input=args.output, output=Path(str(args.output) + "_summary"))
    return 0
