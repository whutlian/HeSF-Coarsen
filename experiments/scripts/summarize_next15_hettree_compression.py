...
METRICS = ["macro_f1", "micro_f1", "accuracy", "actual_ratio", "train_time", "feature_time", "total_time"]
...
def summarize_next15_hettree_compression(*, input: str | Path, output: str | Path) -> dict[str, int]:
    input = Path(input)
    output = Path(output)
    raw_path = input / "hettree_runs.csv"
    if not raw_path.exists():
        raise FileNotFoundError(f"missing required input: {raw_path}")
    output.mkdir(parents=True, exist_ok=True)
    rows = _read_csv(raw_path)
    write_csv(output / "hettree_runs.csv", rows)

    by_dataset = _aggregate(rows, ["dataset", "method", "target_ratio"])
    by_method = _aggregate(rows, ["method", "target_ratio"])
    write_csv(output / "hettree_by_dataset_ratio_method.csv", by_dataset)
    write_csv(output / "hettree_by_ratio_method.csv", by_method)
    ...
    summary = [
        "# Next15 HETTREE Compression Evaluation",
        "",
        "This table reports HETTREE-style downstream evaluation on coarse HGB graphs.",
        "The local evaluator is `hettree_lite` because the official AAAI25 HETTREE repository was not locally available during this run.",
        "",
        "Ratios: 1.2%, 2.4%, 4.8%, 9.6%. Metrics: macro-F1, micro-F1, accuracy.",
        "",
        markdown_table(display_rows, ["dataset", "method", "ratio", "macro_f1", "micro_f1", "accuracy", "n", "failed"]),
        "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summarize_next15_hettree_compression(input=args.input, output=args.output)
    return 0
