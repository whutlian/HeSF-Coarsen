...
BACKENDS: dict[str, dict[str, Any]] = {
    ...
    "A6_direct_relation_writer": {"aggregation_reducer": "direct_relation_writer", "aggregation_chunk_size": 1_000_000},
    "A7_parallel_relation_output_writer": {"aggregation_reducer": "parallel_relation_output_writer", "aggregation_chunk_size": 1_000_000},
    "A8_shard_count_chunk_sweep": {"aggregation_reducer": "shard_count_chunk_sweep", "aggregation_chunk_size": 2_000_000},
}
...
def _run_name(size: str, method: str, backend: str) -> str:
    return f"next14_ogbn_output_merge_{backend}_{size}_{method.replace('-', '_')}"
...
def _rows_from_run(run_dir: Path, size: str, method: str, backend: str, status: str, reason: str = ""):
    ...
    run = {
        ...
        "exclusive_timing_residual_frac": agg.get("exclusive_timing_residual_frac", ""),
        "run_dir": str(run_dir),
    }
    timing_keys = [
    ...
    output_diag = {
        ...
        "method": method,
        "backend": backend,
        "num_shards": agg.get("num_shards", ""),
        "merge_input_files": agg.get("merge_input_files", ""),
        "bytes_written": agg.get("bytes_written", ""),
        "output_write_bytes_per_sec": agg.get("output_write_bytes_per_sec", ""),
        "merge_output_edges": agg.get("merge_output_edges", ""),
        "exclusive_kway_merge_sec": agg.get("exclusive_kway_merge_sec", ""),
        "exclusive_output_write_sec": agg.get("exclusive_output_write_sec", ""),
    }
    ...
    return run, timing, by_relation, output_diag, check


def run_next14_ogbn_output_merge_backend(*, sizes: Sequence[str], methods: Sequence[str], backends: Sequence[str], output: Path, python: str, progress: bool) -> None:
    output.mkdir(parents=True, exist_ok=True)
    runs: list[dict[str, Any]] = []
    timings: list[dict[str, Any]] = []
    rels: list[dict[str, Any]] = []
    output_diags: list[dict[str, Any]] = []
    ...
    for backend in backends:
        ...
        for method in methods:
            for size in sizes:
                ...
                write_command_metadata(run_dir, run_name=run_name, command=command, status="running", size=size, method=method, backend=backend, experiment_block="next14_ogbn_output_merge_backend")
                ...
                write_command_metadata(run_dir, run_name=run_name, command=command, status="success" if completed.returncode == 0 else "failed", returncode=completed.returncode, size=size, method=method, backend=backend, experiment_block="next14_ogbn_output_merge_backend")
                run, timing, rel_rows, output_diag, check = _rows_from_run(run_dir, size, method, backend, status, reason)
                runs.append(run)
                ...
                write_csv(output / "aggregation_output_merge_diagnostics.csv", output_diags)
                write_csv(output / "edge_weight_preservation_checks.csv", checks)
    summarize_next14_ogbn_output_merge_backend(input=output, output=output)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--backends", nargs="+", default=["A0_current_sort_reducer", "A6_direct_relation_writer", "A7_parallel_relation_output_writer", "A8_shard_count_chunk_sweep"])
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--progress", action="store_true")
    args = parser.parse_args(argv)
    run_next14_ogbn_output_merge_backend(sizes=args.sizes, methods=args.methods, backends=args.backends, output=args.output, python=args.python, progress=bool(args.progress))
    return 0
