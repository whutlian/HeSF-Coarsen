...
def _run_fast_selection_method(
    ...
    def occlusion_score(occluded_nodes: np.ndarray) -> dict[str, float]:
        ...
        if key in occlusion_cache:
            return dict(occlusion_cache[key])
        occluded = set(key)
        ...
        payload = _occlusion_metric_payload(
            metrics,
            float(score),
            float(len(key) / max(len(support_nodes_all), 1)),
        )
        occlusion_cache[key] = dict(payload)
        return payload
    ...
    row = {
        ...
        "residual_prototype_mode": graph_diag.get("residual_prototype_mode", str(getattr(cfg.selector, "residual_prototype_mode", ""))),
        "full_residual_upperbound": bool(graph_diag.get("full_residual_upperbound", False)),
        "represented_support_context_count": int(graph_diag.get("represented_support_context_count", 0) or 0),
        ...
        "prototype_member_budget_total": int(graph_diag.get("prototype_member_budget_total", 0) or 0),
        ...
        "validation_positive_fill_count": int(selection_diag.get("validation_positive_fill_count", 0) or 0),
        "validation_neutral_fill_count": int(selection_diag.get("validation_neutral_fill_count", 0) or 0),
        "validation_negative_rejected_count": int(selection_diag.get("validation_negative_rejected_count", 0) or 0),
        ...
        "occlusion_task_nonzero_delta_rate": float(selection_diag.get("occlusion_task_nonzero_delta_rate", 0.0) or 0.0),
        "occlusion_tree_nonzero_delta_rate": float(selection_diag.get("occlusion_tree_nonzero_delta_rate", 0.0) or 0.0),
        "occlusion_task_signal_pass": bool(selection_diag.get("occlusion_task_signal_pass", False)),
        "occlusion_tree_signal_pass": bool(selection_diag.get("occlusion_tree_signal_pass", False)),
        "positive_occlusion_selected_count": int(selection_diag.get("positive_occlusion_selected_count", 0) or 0),
        "neutral_fill_count": int(selection_diag.get("neutral_fill_count", 0) or 0),
        "negative_occlusion_rejected_count": int(selection_diag.get("negative_occlusion_rejected_count", 0) or 0),
